...
def nice_mean(nice, mean, name):
    ...
    while stop:
        ...
        if pick == "n":
            print("\nThe stranfer walks away smiling...")
            nice = (nice + 1)
            stop = False
        if pick == "m":
            print("\nThe stranfer glares at you \nmeancingly and storms off...")
            mean = (mean + 1)
            stop = False
...
def score (nice,mean,name):
    ...
    if nice > 2: # if condition is valid, call win function passing in the variables so it can use them
        win(nice,mean,name)
    if mean > 2:  # if condition is valid, call lose function passing in the varibales so that it can use them
        lose(nice,mean,name)
    else:           # else, call_mean function passing in the variables so it can use them
...
def lose(nice,mean,name):
    # substitute the {} wildcards with our variable values
    print("\nAhhh too bad, game over! \n{}, you live in a dirty beat-up building by the river, wretched and alone!".format(name))
    # call again function and pass in our variables
    again(nice,mean,name)
